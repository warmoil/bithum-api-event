...
"본 스크립트는 MIT 라이선스 혹은 "AS-IS" (있는 그대로) 제공됩니다. 개발자는 이 코드가 특정 이벤트 지원금을 보장한다는 어떠한 명시적/묵시적 보증도 하지 않으며, 본 코드를 사용함으로써 발생하는 어떠한 손해나 지급 반려에 대해서도 책임을 지지 않습니다. 사용자는 이 코드를 사용하기 전에 모든 위험을 스스로 감수함에 동의합니다."
