...
### 이 코드는 빗썸 API 이벤트 참여 메커니즘을 <br> 이해하기 위한 교육 목적으로 제작되었습니다.


## **🚨 중요**: 빗썸 API 이벤트는 지급 반려 사례가 다수 보고되고 있습니다.  <br> 본 스크립트 사용 전 반드시 아래를 확인하세요:

 1. 유튜브/네이버/구글에서 "빗썸 API 이벤트 반려" 검색
 2. 최근 피해 사례 확인
 3. 참여 전 충분히 검토
 4. 현재 실시간으로 유의 사항이 변경되고있습니다.


반드시 이벤트 참여 전에 유튜브, 구글, 네이버 등에서 "빗썸 API 이벤트 지급 반려 사례"를 <br/>
먼저 검색하여 실제로 지원금을 받을 수 있는 상황인지 확인 후 사용하시길 바랍니다.

추가: 실시간으로 반려 사례가 늘어나고있습니다. 
...
이 코드는 "AS-IS" (있는 그대로) 제공되며, 개발자는 특정 이벤트 지원금 지급을 보장한다는 <br>
어떠한 명시적/묵시적 보증도 하지 않습니다. 또한, 본 코드를 사용함으로써 발생하는 거래상의 손해, <br>
API 사용 관련 문제, 또는 이벤트 지급 반려 등 모든 종류의 결과에 대해서도 일절 책임을 지지 않으며,<br>
사용자는 모든 위험을 스스로 감수함에 동의합니다
